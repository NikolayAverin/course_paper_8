...
class IsPleasantHabitValidator:
    ...
    def __call__(self, value):
        ...
        if tmp_is_pleasant_habit and (self.related_habit or self.award):
            raise ValidationError(
                "Если привычка приятная, не может быть связанных привычек или вознаграждения"
            )


class PeriodValidator:
    """Проверка на период выполнения привычки"""
    ...
    def __call__(self, value):
        ...
        if tmp_value:
            if tmp_value > 7:
                raise ValidationError("Период выполнения не может быть больше недели")
